  ...
  return (
    <div className="flex items-center gap-1 px-3 py-1 bg-dark-800 border-b border-dark-600 h-12">
      ...
      <button title="Undo (Ctrl+Z)" onClick={undo} disabled={undoStack.length === 0}
        className="toolbar-btn text-gray-400 disabled:opacity-30"><Undo2 size={18} /></button>
      <button title="Redo (Ctrl+Y)" onClick={redo} disabled={redoStack.length === 0}
        className="toolbar-btn text-gray-400 disabled:opacity-30"><Redo2 size={18} /></button>

      <div className="flex-1" />

      <button title="Save Project" onClick={handleSave} className="toolbar-btn text-gray-400 hover:text-white">
      ...
      <button title="Projects" onClick={onOpenProjects} className="toolbar-btn text-gray-400 hover:text-white">
        <FolderOpen size={18} /><span className="text-xs">Projects</span>
      </button>
      <button title="Export" onClick={onExport} className="toolbar-btn text-accent hover:text-white">
