      ...
      <div className="bg-dark-800 border border-dark-500 rounded-xl p-6 w-96 shadow-2xl">
        ...
        <div className="space-y-4">
            ...
            <div className="flex gap-2">
              {(['png', 'jpeg', 'webp'] as const).map(f => (
                <button key={f} onClick={() => setFormat(f)}
                  className={`flex-1 py-1.5 rounded text-sm ${format === f ? 'bg-accent text-white' : 'bg-dark-600 text-gray-300'}`}>
                  {f.toUpperCase()}
                </button>
              ))}
            </div>
          ...
          {format !== 'png' && (
            <div>
              <div className="flex justify-between text-xs text-gray-400 mb-1">
                <span>Quality</span><span className="text-white">{quality}%</span>
              </div>
          ...
          <div className="text-xs text-gray-500">
            Output: {canvasWidth} × {canvasHeight}px
            {upscale && ` → ${canvasWidth * upscaleFactor} × ${canvasHeight * upscaleFactor}px`}
          </div>

          {/* Upscale */}
          <div className="flex items-center gap-3">
            <input type="checkbox" id="upscale" checked={upscale}
              onChange={e => setUpscale(e.target.checked)} className="accent-accent" />
            <label htmlFor="upscale" className="text-sm text-gray-300">Upscale (Real-ESRGAN)</label>
            {upscale && (
